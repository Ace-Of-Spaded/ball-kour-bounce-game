...
class SafePlatform {
	...
	render() {
		ctx.save();
		ctx.fillStyle = `hsla(${this.color}, 100%, 55%, 1)`;
		ctx.fillRect(this.x, this.y, this.width, this.height);
		ctx.restore();
	}
}
...
class PlatformManager {
	...
	update() {
		...
		while (furthestX < canvas.width * 2) {
			...
			let spacerY = Math.floor(Math.random() * 150 + 300);

			let nextPlatformType = Math.random();

			let p;

			if (nextPlatformType > 0.1 && nextPlatformType < 0.2) {
				p = new SafePlatform(this.game);
			} else if (nextPlatformType > 0.2 && nextPlatformType < 0.5)
			{
